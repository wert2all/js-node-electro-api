...
export default class UserProfileFetchModel {
    ...
    async extendResponseUserProfile(response, requestData) {
        const userProfileList = await this._fetchUserProfile(requestData.getGoogleUserId());
    ...
    async _fetchUserProfile(googleUserId) {
        ...
        const userEntity = new UserEntity().setValue(UserDefinition.COLUMN_GOOGLE_ID, googleUserId);
